-- ~78 ms
WITH tmp (Group_id, Category_id, num) as (SELECT R2.Group_id, B.Category_id, Count(*) as num
                                          FROM Issues
...
FROM tmp t
INNER JOIN Groups G on t.Group_id = G.Group_id INNER JOIN Category C ON t.Category_id = C.Category_id
