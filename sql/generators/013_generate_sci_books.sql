...
CREATE OR ALTER PROCEDURE GenerateSciBooks @publisher_per_book INT
AS
BEGIN
    ...
    DECLARE @category_id INT = (SELECT Category_id FROM Category WHERE Category_name = N'Научная литература')
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            DECLARE @subject_id INT = ABS(CHECKSUM(NEWID())) % @subject_count + 1
            WHILE @i < @publisher_per_book
                BEGIN
                    ...
                    VALUES (@name, @author_id, @publisher_id, @subject_id, @category_id)
                    SET @i = @i + 1
                end
            FETCH NEXT FROM names_cursor INTO @name
        end
