CREATE OR REPLACE PROCEDURE generate_sci_books(publishers_per_book INT)
AS
$$
DECLARE
    _book_name        VARCHAR;
...
BEGIN
    ...
    COPY _books_staging (name)
        FROM '/import/uni-library/data/science_books.csv'
        DELIMITER ';'
        CSV HEADER;
    ...
    _category_id = (SELECT category_id FROM category WHERE category_name = N'Научная литература');
